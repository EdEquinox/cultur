...
|------------------|--------------|-----|
| `cultur` @ `eqnox.pt` | HTTP | `http://192.168.1.230:8081` (web) |
| `api.cultur` @ `eqnox.pt` | HTTP | `http://192.168.1.230:8787` (API) |

Same pattern as `music.eqnox.pt` → `192.168.1.230:4533` — **one port per app**, no Caddy in the Portainer stack.
