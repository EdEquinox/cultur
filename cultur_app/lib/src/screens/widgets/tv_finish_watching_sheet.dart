...
enum TvWatchingOutcome { finished, dropped, paused }

enum _ActionWhenOption { now, custom }

class TvFinishWatchingSheetResult {
  const TvFinishWatchingSheetResult({
    required this.outcome,
    this.score,
    this.actionAtUtc,
  });

  final TvWatchingOutcome outcome;
  final double? score;
  final DateTime? actionAtUtc;
}

/// Shown when stopping an in-progress TV series.
class TvFinishWatchingSheet extends StatefulWidget {
  const TvFinishWatchingSheet({
    super.key,
  ...
  @override
  State<TvFinishWatchingSheet> createState() => _TvFinishWatchingSheetState();
}

class _TvFinishWatchingSheetState extends State<TvFinishWatchingSheet> {
  TvWatchingOutcome? _outcome;
  late int _stars;
  ...
  DateTime? get _actionAtUtc {
    if (_outcome == TvWatchingOutcome.paused) {
      return null;
    }
    return switch (_when) {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if (widget.seriesTitle.trim().isNotEmpty) ...[
            ...
            _OutcomeTile(
              selected: _outcome == TvWatchingOutcome.finished,
              icon: Icons.check_circle_outline,
              selectedIcon: Icons.check_circle,
              label: 'Finished the series',
              subtitle: 'Moves to Finished',
              onTap: () => setState(() => _outcome = TvWatchingOutcome.finished),
            ),
            const SizedBox(height: 8),
            _OutcomeTile(
              selected: _outcome == TvWatchingOutcome.dropped,
              icon: Icons.flag_outlined,
              selectedIcon: Icons.flag,
              label: 'Stopped watching',
              subtitle: 'Moves to Left',
              onTap: () => setState(() => _outcome = TvWatchingOutcome.dropped),
            ),
            const SizedBox(height: 8),
            _OutcomeTile(
              selected: _outcome == TvWatchingOutcome.paused,
              icon: Icons.pause_circle_outline,
              selectedIcon: Icons.pause_circle,
              label: 'Pause for now',
              subtitle: 'Back to Later',
              onTap: () => setState(() => _outcome = TvWatchingOutcome.paused),
            ),
            if (_outcome != TvWatchingOutcome.paused) ...[
                    ...
                    onPressed: _outcome == null
                        ...
                        : () {
                            Navigator.of(context).pop(
                              TvFinishWatchingSheetResult(
                                outcome: _outcome!,
                                score: _outcome != TvWatchingOutcome.paused && _stars > 0
                                    ? _stars.toDouble()
                                    : null,
