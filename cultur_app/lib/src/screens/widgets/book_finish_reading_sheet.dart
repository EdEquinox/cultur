...
enum BookReadingOutcome { read, dropped, paused }

enum _ActionWhenOption { now, custom }

class BookFinishReadingSheetResult {
  const BookFinishReadingSheetResult({
    required this.outcome,
    this.score,
    this.actionAtUtc,
  });

  final BookReadingOutcome outcome;
  final double? score;
  final DateTime? actionAtUtc;
}

/// Shown when stopping an in-progress book: read, left, or pause back to Later.
class BookFinishReadingSheet extends StatefulWidget {
  const BookFinishReadingSheet({
    super.key,
  ...
  @override
  State<BookFinishReadingSheet> createState() => _BookFinishReadingSheetState();
}

class _BookFinishReadingSheetState extends State<BookFinishReadingSheet> {
  BookReadingOutcome? _outcome;
  late int _stars;
  ...
  DateTime? get _actionAtUtc {
    if (_outcome == BookReadingOutcome.paused) {
      return null;
    }
    return switch (_when) {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _OutcomeTile(
              selected: _outcome == BookReadingOutcome.read,
              icon: Icons.check_circle_outline,
              selectedIcon: Icons.check_circle,
              label: 'Finished reading',
              subtitle: 'Moves to Read',
              onTap: () => setState(() => _outcome = BookReadingOutcome.read),
            ),
            const SizedBox(height: 8),
            _OutcomeTile(
              selected: _outcome == BookReadingOutcome.dropped,
              icon: Icons.flag_outlined,
              selectedIcon: Icons.flag,
              label: 'Stopped / left it',
              subtitle: 'Moves to Left',
              onTap: () => setState(() => _outcome = BookReadingOutcome.dropped),
            ),
            const SizedBox(height: 8),
            _OutcomeTile(
              selected: _outcome == BookReadingOutcome.paused,
              icon: Icons.pause_circle_outline,
              selectedIcon: Icons.pause_circle,
              label: 'Pause for now',
              subtitle: 'Back to Later',
              onTap: () => setState(() => _outcome = BookReadingOutcome.paused),
            ),
